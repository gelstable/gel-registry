...
def create_immutable(path: Path, data: bytes, label: str) -> bool:
    """Create one immutable file, tolerating a replay of identical bytes.

    Returns whether the file was created.  A destination already holding these
    exact bytes is not a collision -- that is what lets an interrupted
    publication be re-run -- while differing bytes are refused outright.
    """

    try:
        return storage.create_atomic(path, data)
    except ValueError as exc:
        raise RenderError(f"immutable {label} mismatch: {exc}") from exc
    except OSError as exc:
        raise RenderError(f"could not install immutable {label} {path}: {exc}") from exc
