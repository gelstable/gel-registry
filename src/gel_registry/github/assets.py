...
from ..constants import MAX_ASSET_BYTES, MAX_TOTAL_ASSET_BYTES
...
class DeterministicAssetError(GitHubError):
    """Raised for a reproducible upstream asset defect."""

    def __init__(self, message: str, *, assets: Iterable[str] = ()) -> None:
...
def download_assets(
    ...
    *,
    ...
    """Stream selected published release assets into a new temporary destination.
    ...
    selected_names = (
        tuple(asset.name for asset in release.assets) if names is None else tuple(names)
    )
    if len(selected_names) != len(set(selected_names)):
        raise GitHubError("requested asset names contain duplicates")
    missing = sorted(set(selected_names) - set(asset_by_name))
    if missing:
        raise GitHubError(
            "requested asset names are missing from release: " + ", ".join(missing)
        )
    selected_assets = tuple(asset_by_name[name] for name in selected_names)
    declared_total = 0
    for asset in selected_assets:
        if asset.size is not None and asset.size > MAX_ASSET_BYTES:
            raise DeterministicAssetError(
                f"asset {asset.name!r} exceeds maximum size of {MAX_ASSET_BYTES} bytes",
                assets=(asset.name,),
            )
    ...
    if declared_total > MAX_TOTAL_ASSET_BYTES:
        raise DeterministicAssetError(
            "declared aggregate asset size exceeds maximum of "
            f"{MAX_TOTAL_ASSET_BYTES} bytes",
            assets=selected_names,
        )
    ...
    try:
        ...
        aggregate_size = 0
        for asset in selected_assets:
            url = _download_url_for_asset(asset, repository)
                        ...
                        with target.open("wb") as stream:
                            for chunk in response.iter_bytes(
                                ...
                                if size + len(chunk) > MAX_ASSET_BYTES:
                                    raise DeterministicAssetError(
                                        f"asset {asset.name!r} exceeds maximum size of "
                                        f"{MAX_ASSET_BYTES} bytes",
                                        assets=(asset.name,),
                                    )
                                if aggregate_size + len(chunk) > MAX_TOTAL_ASSET_BYTES:
                                    raise DeterministicAssetError(
                                        "streamed aggregate asset size exceeds "
                                        f"maximum of {MAX_TOTAL_ASSET_BYTES} bytes",
                                        assets=selected_names,
                                    )
                                stream.write(chunk)
                                size += len(chunk)
...
def _allowed_asset_api_url(url: str, repository: str) -> bool:
    try:
        parsed = urlsplit(url)
    except ValueError:
        return False
    ...
    if parsed.fragment or parsed.query:
        return False
    prefix = f"/repos/{repository}/assets/"
    return parsed.path.startswith(prefix) and parsed.path.removeprefix(prefix).isdigit()


def _download_url_for_asset(asset: GitHubAsset, repository: str) -> str:
    if _allowed_asset_api_url(asset.url, repository):
        return asset.url
    ...
    if (
        ...
        return asset.url
    raise DeterministicAssetError(
        f"asset {asset.name!r} is not an allowlisted GitHub asset URL",
        assets=(asset.name,),
    )
