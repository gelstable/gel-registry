...
class GitHubRelease(BaseModel):
    ...
    @classmethod
    def parse_payload(cls, value: object, info: ValidationInfo[object]) -> object:
        ...
        context_repository = (
            info.context.get("repository")
            if isinstance(info.context, Mapping)
            else None
        )
        data["repository"] = (
            context_repository
            if isinstance(context_repository, str) and "/" in context_repository
            else sorted(repositories)[0]
            if repositories
